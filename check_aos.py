            ...
            if app_id[1] == True:
                ...
                if soup.find('script', type="application/ld+json") is None:
                    ...
                    text="{icon_url}\n<https://play.google.com/store/apps/details?id={app_id} | {app_name}> の android アプリが非公開になりました\nジャンル: {genre}\nコンテンツレーティング: {content_rating}\nデベロッパー: <https://play.google.com/store/apps/developer?id={publisher_id} | {publisher_name}>\nインストール数: {installs}\nビデオ: {video}\n"\
                            .format(icon_url=app[0]['icon_url'], app_id=app[0]['app_id'], app_name=app[0]['app_name'], genre=app[0]['genre'], content_rating=app[0]['content_rating'], publisher_id=app[0]['publisher_id'], publisher_name=app[0]['publisher_name'], rating=app[0]['rating'], rating_count=app[0]['rating_count'], reviews=app[0]['reviews'], description=app[0]['description'], video=app[0]['video'], installs=app[0]['installs'])
...
with open(LOG, mode='a') as f:
    f.write(str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+": check_aos\n")
