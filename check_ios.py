...
with get_connection() as conn:
    with conn.cursor() as cur:
        cur.execute("SELECT app_id, is_release FROM superset_schema.app_details WHERE platform = 2;")
            ...
            if app_id[1] == True:
                ...
                if app_dict['resultCount'] == 0:
                    ...
                    if app[0]['screenshots'] is not None:
                        for i,s in enumerate(json.loads(app[0]['screenshots'])):
                            if i < 3:
                                ...
                                SCREENSHOTS2=SCREENSHOTS2 + "<" + s + "|" + str(i) + "> "
                    ...
                    text="{icon_url}\n{app_name} の iPhone アプリが非公開になりました\nジャンル: {genre}\nコンテンツレーティング: {content_rating}\nデベロッパー: <https://apps.apple.com/us/developer/{publisher_name_2}/id{publisher_id} | {publisher_name}>\nインストール数: {installs}\n"\
                            .format(icon_url=app[0]['icon_url'], app_id=app[0]['app_id'], app_name=app[0]['app_name'], genre=app[0]['genre'], content_rating=app[0]['content_rating'], publisher_id=app[0]['publisher_id'], publisher_name=app[0]['publisher_name'], publisher_name_2=PUBLISHER_NAME, rating=app[0]['rating'], rating_count=app[0]['rating_count'], reviews=app[0]['reviews'], description=app[0]['description'], video=app[0]['video'], installs=app[0]['installs'])
                    ...
                    if SCREENSHOTS != "":
                        requests.post("https://hooks.slack.com/services/" + SLACK_URL, data = json.dumps({
                            'text': "スクリーンショット: " + SCREENSHOTS,  #通知内容
                            'username': u'確認',  #ユーザー名
                            'icon_emoji': u':smile_cat:',  #アイコン
                            'link_names': 1,  #名前をリンク化
                        }))
